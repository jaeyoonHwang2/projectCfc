...
public:
	...
	uint8_t GetPortIndex(){ //// cfc: to return output port index
		return outputPortIndex;
	}
	void Set(uint64_t _time, uint64_t _bytes, uint32_t _qlen, uint64_t _rate, uint64_t _portIdx){
		time = _time;
		bytes = _bytes / (byteUnit * multi);
		qlen = _qlen / (qlenUnit * multi);
		switch (_rate){
			...
			default:
				printf("Error: IntHeader unknown rate: %lu\n", _rate);
				break;
		outputPortIndex = _portIdx;
		}
	}
	uint64_t GetBytesDelta(CfcIntHop &b){
		if (bytes >= b.bytes)
			return (bytes - b.bytes) * byteUnit * multi;
		else
			return (bytes + (1<<bytesWidth) - b.bytes) * byteUnit * multi;
	}
	uint64_t GetTimeDelta(CfcIntHop &b){
		if (time >= b.time)
			return time - b.time;
		else
			return time + (1<<timeWidth) - b.time;
...
public:
	...
	//// cfc
	void CfcPushHop(uint64_t time, uint64_t bytes, uint32_t qlen, uint64_t rate, uint8_t portIdx);
	////
	...
	//// cfc
	// std::ofstream DebugIntHeader;
	// std::string filePath;
	// void PrintOutputPorts(void);
	////
