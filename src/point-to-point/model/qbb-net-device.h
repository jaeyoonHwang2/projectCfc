...
public:
  ...
  //// CFC
  std::ofstream writeFile_rdmaEgressQueue;
  std::string filePath_rdmaEgressQueue;
  ////
};
...
protected:
  ...
  //// CFC: Fair Queueing Mechanism
  uint32_t lastDequeueIndex;  // Tracks the last queue index that was dequeued
  uint32_t totalQueues;       // Total number of queues

  void ScheduleNextDequeue();
...
public:
  ...
  //// CFC
  std::string filePath;
  std::ofstream writeFile_qbbNetDevice;
	////
