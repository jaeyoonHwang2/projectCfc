...
void RdmaHw::SetNode(Ptr<Node> node){
	// cfc print
	filePath = "/home/nxc/proj_alpha_cfc/simulation/RdmaHw.txt";
	writeFile_rdmaHw.open(filePath.c_str(), std::ios::app);
	filePath = "/home/nxc/proj_alpha_cfc/simulation/Qmonopoly_rate.txt";
	Qmonopoly_rate.open(filePath.c_str(), std::ios::app);
	filePath = "/home/nxc/proj_alpha_cfc/simulation/Qshare1_rate.txt";
	Qshare1_rate.open(filePath.c_str(), std::ios::app);
	filePath = "/home/nxc/proj_alpha_cfc/simulation/Qshare2_rate.txt";
	Qshare2_rate.open(filePath.c_str(), std::ios::app);
...
void RdmaHw::MeasureThr(Ptr<Packet> p, CustomHeader &ch){ // cfc: measure throughput (per-flow)
	measureGran_ns = 12000;// measurement granularity
	measurePeriod = (double) 1000000000 / measureGran_ns; 
	//// check flow
	// writeFile_rdmaHw << "ch.dip / ch.sip / ch.udp.dport / ch.udp.sport: " << ch.dip << " " << ch.sip << " " << ch.udp.dport << " " << ch.udp.sport << "\n";
	if(ch.dip == 184551937 && ch.sip == 184551425){ // specify target flows
		curTstamp = (Simulator::Now().GetNanoSeconds() - 2000000000) / measureGran_ns;
		if (lastTstamp == 0){
			lastTstamp = curTstamp;
		}
		while (curTstamp > lastTstamp){
			if(writeFile_rdmaHw.is_open()){
				writeFile_rdmaHw << "period " << lastTstamp * measureGran_ns << " throughput " << (double) sumBytes * 8 * measurePeriod / (1024 * 1024 * 1024) << "\n";
				sumBytes = 0;
				lastTstamp += 1;
	...
	if (qp->hp.m_lastUpdateSeq == 0){ // first RTT
		...
		if (ih.nhop <= IntHeader::maxHop){
			...
			for (uint32_t i = 0; i < ih.nhop; i++){
				...
				// best place to print INT information
				#if PRINT_LOG
				if (print)
 ...
 * CFC
 ***********************/
void RdmaHw::HandleAckCfc(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	uint32_t ack_seq = ch.ack.seq;
	// update rate
	if (ack_seq > qp->cfc.m_lastUpdateSeq){ // if full RTT feedback is ready, do full update
		UpdateRateCfc(qp, p, ch, false);
	}else{ // do fast react
		FastReactCfc(qp, p, ch);
	}
	// RATE IS UPDATED									
}

void RdmaHw::UpdateRateCfc(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
	uint32_t next_seq = qp->snd_nxt;
	bool print = !fast_react || true;
	if (qp->cfc.m_lastUpdateSeq == 0){ // first RTT
		qp->cfc.m_lastUpdateSeq = next_seq;	
		// store INT
		IntHeader &ih = ch.ack.ih;
		NS_ASSERT(ih.cfc_nhop <= IntHeader::maxHop);
		for (uint32_t i = 0; i < ih.cfc_nhop; i++)
			qp->cfc.hop[i] = ih.cfc_hop[i];
		#if PRINT_LOG
		if (print){
			printf("%lu %s %08x %08x %u %u [%u,%u,%u]", Simulator::Now().GetTimeStep(), fast_react? "fast" : "update", qp->sip.Get(), qp->dip.Get(), qp->sport, qp->dport, qp->hp.m_lastUpdateSeq, ch.ack.seq, next_seq);
			for (uint32_t i = 0; i < ih.nhop; i++)
		...
		IntHeader &ih = ch.ack.ih;
		if (ih.cfc_nhop <= IntHeader::maxHop){
			if(Qmonopoly_rate.is_open()){
				Qmonopoly_rate << "Time to check hops\n";
				Qmonopoly_rate << "total number of hops: " << ih.cfc_nhop << "\n";
				Qmonopoly_rate.flush();
			}	
			double max_c = 0;
			bool inStable = false;
			#if PRINT_LOG
			...
			for (uint32_t i = 0; i < ih.cfc_nhop; i++){
				if (m_sampleFeedback){
					if (ih.cfc_hop[i].GetQlen() == 0 && fast_react)
						continue;
				}
				updated[i] = updated_any = true;
				#if PRINT_LOG
				if (print)
					printf(" %u(%u) %lu(%lu) %lu(%lu)", ih.hop[i].GetQlen(), qp->hp.hop[i].GetQlen(), ih.hop[i].GetBytes(), qp->hp.hop[i].GetBytes(), ih.hop[i].GetTime(), qp->hp.hop[i].GetTime());
				#endif
				uint64_t tau = ih.cfc_hop[i].GetTimeDelta(qp->cfc.hop[i]);;
				double duration = tau * 1e-9;
				double txRate = (ih.cfc_hop[i].GetBytesDelta(qp->cfc.hop[i])) * 8 / duration;
				double u = txRate / ih.cfc_hop[i].GetLineRate() + (double)std::min(ih.cfc_hop[i].GetQlen(), qp->cfc.hop[i].GetQlen()) * qp->m_max_rate.GetBitRate() / ih.hop[i].GetLineRate() /qp->m_win;
				uint8_t idx = ih.cfc_hop[i].GetPortIndex();
				if(Qmonopoly_rate.is_open()){
					Qmonopoly_rate << "hop" << i << "idx " << idx << "\n";
					Qmonopoly_rate.flush();
				}	
				// best place to print INT information
				#if PRINT_LOG
				...
				if (!m_multipleRate){
					...
					if (tau > qp->m_baseRtt)
						tau = qp->m_baseRtt;
					qp->cfc.hopState[i].u = (qp->cfc.hopState[i].u * (qp->m_baseRtt - tau) + u * tau) / double(qp->m_baseRtt);
				}
				qp->cfc.hop[i] = ih.cfc_hop[i];
			}
			...
			if (!m_multipleRate){
				...
				if (updated_any){
					if (dt > qp->m_baseRtt)
						dt = qp->m_baseRtt;
					qp->cfc.u = (qp->cfc.u * (qp->m_baseRtt - dt) + U * dt) / double(qp->m_baseRtt);
					max_c = qp->cfc.u / m_targetUtil;

					if (max_c >= 1 || qp->cfc.m_incStage >= m_miThresh){
						new_rate = qp->cfc.m_curRate / max_c + m_rai;
						new_incStage = 0;
					}else{
						new_rate = qp->cfc.m_curRate + m_rai;
						new_incStage = qp->cfc.m_incStage+1;
					}
					if (new_rate < m_minRate)
						new_rate = m_minRate;
					if (new_rate > qp->m_max_rate)
				...
				for (uint32_t i = 0; i < ih.cfc_nhop; i++){
					if (updated[i]){
						double c = qp->cfc.hopState[i].u / m_targetUtil;
						if (c >= 1 || qp->cfc.hopState[i].incStage >= m_miThresh){
							new_rate_per_hop[i] = qp->cfc.hopState[i].Rc / c + m_rai;
							new_incStage_per_hop[i] = 0;
						}else{
							new_rate_per_hop[i] = qp->cfc.hopState[i].Rc + m_rai;
							new_incStage_per_hop[i] = qp->cfc.hopState[i].incStage+1;
						}
						// bound rate
						if (new_rate_per_hop[i] < m_minRate)
							new_rate_per_hop[i] = m_minRate;
						...
						if (qp->cfc.hopState[i].Rc < new_rate)
							new_rate = qp->cfc.hopState[i].Rc;
					}
				}
				#if PRINT_LOG
				printf("\n");
			...
			if (!fast_react){
				if (updated_any){
					qp->cfc.m_curRate = new_rate;
					qp->cfc.m_incStage = new_incStage;
				}
				if (m_multipleRate){
					// for per hop (per hop R)
					for (uint32_t i = 0; i < ih.cfc_nhop; i++){
						if (updated[i]){
							qp->cfc.hopState[i].Rc = new_rate_per_hop[i];
							qp->cfc.hopState[i].incStage = new_incStage_per_hop[i];
						}
					}
				}
			}
		...
		if (!fast_react){
			if (next_seq > qp->cfc.m_lastUpdateSeq)
				qp->cfc.m_lastUpdateSeq = next_seq; //+ rand() % 2 * m_mtu;
		}
	}
}

void RdmaHw::FastReactCfc(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	if (m_fast_react)
		UpdateRateCfc(qp, p, ch, true);
}
