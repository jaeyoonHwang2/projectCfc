...
public:
    ...
    double addValue(double val) {
        ...
        // If the window is larger than the windowSize, remove the oldest element
        if (window.size() > 10) {
            window.pop_front();
        }
	...
	int64_t lastTstamp;
	int64_t curTstamp;
	int64_t measureGran_ns;
	double measurePeriod;
	void MeasureThr(Ptr<Packet> p, CustomHeader &ch);
	void HandleAckCfc(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
	void UpdateRateCfc(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react);
	void UpdateRateCfcTest(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react);
	void FastReactCfc(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);

	std::string filePath;
	std::ofstream writeFile_rdmaHw;
	std::ofstream Qmonopoly_rate;
	std::ofstream Qshare1_rate;
	std::ofstream Qshare2_rate;
