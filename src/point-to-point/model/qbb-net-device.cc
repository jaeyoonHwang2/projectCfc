...
namespace ns3 {
	...
	RdmaEgressQueue::RdmaEgressQueue(){
		...
		// initial writeFile
		filePath_rdmaEgressQueue = "/home/nxc/proj_alpha_cfc/simulation/rdmaEgressQueue.txt";
		writeFile_rdmaEgressQueue.open(filePath_rdmaEgressQueue.c_str(), std::ios::app);
	}

	//// PFC
	Ptr<Packet> RdmaEgressQueue::DequeueQindex(int qIndex){
		if(writeFile_rdmaEgressQueue.is_open()){
			writeFile_rdmaEgressQueue << "queue index is" << qIndex << "\n";
			writeFile_rdmaEgressQueue.flush();
		}			
		if (qIndex == -1){ // high prio for ack
			Ptr<Packet> p = m_ackQ->Dequeue();
			m_qlast = -1;
			m_traceRdmaDequeue(p, 0);
			return p;
		}
		if (qIndex >= 0){ // qp; data packet get >= 0 prio
	...
	//// CFC
	// Ptr<Packet> RdmaEgressQueue::DequeueQindex(int lastDequeueIndex) {
	// 	for (uint32_t i = 0; i < qCnt; ++i) {
	// 		// Use round-robin to select the next queue
	// 		uint32_t qIndex = (lastDequeueIndex + i) % qCnt;

	// 		// Check if the queue has packets and is not paused
	// 		if (!paused[qIndex] && !portQueues[qIndex].empty()) {
	// 			Ptr<Packet> packet = portQueues[qIndex].front();
	// 			portQueues[qIndex].pop();
	// 			lastDequeueIndex = qIndex;  // Update the last dequeued index
	// 			return packet;
	// 		}
	// 	}
	// 	return nullptr; // No packets to dequeue
	// }
	////
	...
	QbbNetDevice::QbbNetDevice()
	{
		...
		// initial writeFile
		filePath = "/home/nxc/proj_alpha_cfc/simulation/qbbNetDevice.txt";
		writeFile_qbbNetDevice.open(filePath.c_str(), std::ios::app);
	}
	...
	void QbbNetDevice::DequeueAndTransmit(void)	{
		...
		Ptr<Packet> p;
		if (m_node->GetNodeType() == 0){ // Nodetype: host
			...
			if (p != 0){ // qIndex 0-7 is effective
				m_snifferTrace(p);
	...
	bool QbbNetDevice::SwitchSend (uint32_t qIndex, Ptr<Packet> packet, CustomHeader &ch){
		...
		if(writeFile_qbbNetDevice.is_open()){
			writeFile_qbbNetDevice << "queue index @ SwitchSend is " << qIndex << "\n";
			writeFile_qbbNetDevice.flush();
		}	
		DequeueAndTransmit();
		return true;
	}
