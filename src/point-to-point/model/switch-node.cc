...
SwitchNode::SwitchNode(){
	...
	filePath = "/home/nxc/proj_alpha_cfc/simulation/debugSwitchNode.txt";
	DebugSwitchNode.open(filePath.c_str(), std::ios::app);
	filePath = "/home/nxc/proj_alpha_cfc/simulation/monitorLinkUtilization.txt";
	MonitorLinkUtilization.open(filePath.c_str(), std::ios::app);
	////
}
...
// (likely) ingress -> egress
void SwitchNode::SendToDev(Ptr<Packet>p, CustomHeader &ch){ 

	int idx = GetOutDev(p, ch); // idx: choose egress port index 
	...
	if (idx >= 0){
		...
		} // if TCP, put to queue 1; frankly speaking, a flow can determine its qIndex in the flow generating file
		...
		MeasureUtil(p, ch, idx);

	}else
		return; // Drop
}

// cfc: measure link utilizaiton (per-switch)
void SwitchNode::MeasureUtil(Ptr<Packet> p, CustomHeader &ch, int idx){ 
	measureGran_ns = 10000; // measurement granularity
	measurePeriod = (double) 1000000000 / measureGran_ns; 	
	if(this->GetId() == 0){ // specify switch node id
		...
		while (curTstamp > lastTstamp){
			int i = 0;
			if(MonitorLinkUtilization.is_open()){
				MonitorLinkUtilization << "switchId " << this->GetId() << " period " << lastTstamp * measureGran_ns << " ";
				while (i < pCnt){					
					MonitorLinkUtilization << (double) sumBytes[i] * 8 * measurePeriod / (1024 * 1024 * 1024) << " ";
					sumBytes[i] = 0;
					i += 1;
				}				
				MonitorLinkUtilization << "\n";
				lastTstamp += 1;
				MonitorLinkUtilization.flush();
			}	
		}		
		sumBytes[idx] += p->GetSize();
	}
}
//
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p){
	...
	if (1){
		...
		if (buf[PppHeader::GetStaticSize() + 9] == 0x11){ // udp packet
			...
			if (m_ccMode == 3){ // HPCC
			...
			}else if (m_ccMode == 5){ // CFC
				ih->CfcPushHop(Simulator::Now().GetTimeStep(), m_txBytes[ifIndex], dev->GetQueue()->GetNBytesTotal(), dev->GetDataRate().GetBitRate(), (uint8_t) ifIndex);						
				if(DebugSwitchNode.is_open()){
					DebugSwitchNode << "CfcPushHop probably activated\n";
					DebugSwitchNode << "With Set portIdx be " << ifIndex << "\n";
					DebugSwitchNode.flush();
				}
				...
				//// cfc
				if(DebugSwitchNode.is_open()){ 
					DebugSwitchNode << "Confirm PushHop-ed " << ih->cfc_hop[0].GetPortIndex() << "\n";
					DebugSwitchNode.flush();
				} 
				// ih->PrintOutputPorts();
				////
